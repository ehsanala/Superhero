...
router.get('/heroes', async (req, res) => {
  ...
  // const result = await Hero.find({}).sort({}).skip((page-1)*limit).limit(limit)
  // res.send(result)
  let { page = 1, size = DEFAULT_HEROES_PER_PAGE } = req.params
  page = parseInt(page)
  size = parseInt(size)
  const query = {}
...
router.get('/heroes/:id', async (req, res) => {
  // TODO: fill out the code for the endpoint
  const data = await Hero.findById(req.params.id).exec();
  res.send({data})
...
router.post('/search/heroes/by-name', async (req, res) => {
  ...
  page = parseInt(page)
  size = parseInt(size)
  const query = { name: new RegExp('^'+req.body.query+'$', "i") }

  const totalData = await Hero.find(query).estimatedDocumentCount()
...
router.post('/search/heroes/by-min-stats', async (req, res) => {
  ...
  const query ={
    "powerstats.speed": { $gte: speed },
    "powerstats.intelligence": {$gte: intelligence}
  }
  const totalData = await Hero.find(query).estimatedDocumentCount()
  const data = await Hero.find(query).skip((page - 1) * size).limit(size).exec()
...
router.post('/heroes/:id/comments', async (req, res) => {
  ...
  const comment = new Comment({
    hero: mongoose.Types.ObjectId(id),
    text: req.body.text
  })
  await comment.save()
  await comment.populate("hero")
...
router.get('/heroes/:id/comments', async (req, res) => {
  // TODO: fill out the code for the endpoint
  let { page = 1, size = DEFAULT_COMMENTS_PER_PAGE, id} = req.params
  page = parseInt(page)
  size = parseInt(size)
  const query = { hero: mongoose.Types.ObjectId(id) }

  const totalData = await Comment.find(query).estimatedDocumentCount()
  const data = await Comment.find(query).populate("hero").sort({'updatedAt': 1}).skip((page - 1) * size).limit(size).exec()
